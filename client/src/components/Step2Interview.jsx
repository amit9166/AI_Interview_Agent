...
const Step2Interview = ({interviewData,onFinish}) => {
  ...
  const speakText=(text)=>{
    return new Promise((resolve)=>{
      //If no text-to-speech support or no selected voice: Finish immediately.
      if(!window.speechSynthesis ||!selectedVoice){
        resolve(); 
        return;
      }
      //Stops any current speaking.
  ...
  useEffect(()=>{
    ...
    async function runIntro(){
      if(isIntroPhase){
        ...
        await speakText("I will ask you a few questions. just answer naturally, and take your time. So let's begin.");
  ...
  },[isAIPlaying,isIntroPhase,currentIndex])
...
//the browser’s Web Speech API (webkitSpeechRecognition). 
// Used to create speech recognition instance once.
  useEffect(()=>{
...
//Only final confirmed text. No live partial text like: i ha.., i have..
...
useEffect(() => {
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-emerald-100 via-white to-teal-100 flex
    items-center justify-center p-4 sm:p-6'>
      ...
      shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        {/* video section  */}
        <div className='w-full lg:w-[35%] bg-white flex flex-col items-center p-6 space-y-6 border-r border-gray-300'>
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          {!isIntroPhase && <div className='relative mb-6 bg-gray-50 sm:p-6 rounded-2xl border border-gray-200 shadow-sm'>
            <p className='text-xs sm:text-sm text-gray-400'>Question {currentIndex+1} of {questions.length}</p>
            <div className='text-base sm:text-lg font-semibold text-gray-800 leading-relaxed'>{currentQuestion?.question}</div>
          </div>}
          <textarea className="flex-1 bg-gray-100 p-4 sm:p-6 rounded-2xl resize-none
          ...
          {!feedback ? (<div className='flex items-center gap-4 mt-6'>
            <motion.button 
            disabled={hasSubmitted || isAIPlaying}
            onClick={toggleMic}
            whileTap={{scale:0.9}}
