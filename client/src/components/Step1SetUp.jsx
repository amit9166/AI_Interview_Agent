          ...
          <div className="space-y-5">
            {
              [
                ...
                rounded-xl shadow-sm cursor-pointer" key={index}>
                  {item.icon}
        ...
        className="p-12 bg-white">
          ...
          <div className="space-y-6">
            <div className="relative">
              ...
              <input type="text" placeholder="Enter role" className="w-full pl-12 pr-4 py-3 border border-gray-200
              rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition"
              onChange={(e)=>setRole(e.target.value)} value={role}/>
            </div>
            <div className="relative">
              <FaBriefcase className="absolute top-4 left-4 text-gray-400"/>
              <input type="text" placeholder="Experience" className="w-full pl-12 pr-4 py-3 border border-gray-200
              rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition"
              onChange={(e)=>setExperience(e.target.value)} value={experience}/>
            </div>
            <select value={mode} onChange={(e)=>setMode(e.target.value)} className="w-full py-3 px-4 border border-gray-200
            rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition">
              <option value="Techincal">Techincal Interview</option>
              <option value="HR">HR Interview</option>
            </select>
            {!analysisDone && (
              <motion.div 
              onClick={()=>document.getElementById("resumeUpload").click()} className="border-2 border-dashed border-gray-300
              rounded-xl p-8 text-center cursor-pointer hover:border-green-500
              transition hover:bg-green-50">
                <FaFileUpload className="text-4xl mx-auto text-green-600 mb-3"/>
                <input  onChange={(e)=>setResumeFile(e.target.files[0])} type="file" accept="application/pdf" id="resumeUpload" className="hidden"/>
                ...
                className="mt-4 bg-gray-900 text-white px-5 py-2 rounded-lg hover:bg-gray-800 transition">
                  {analyzing ? "Analyzing...":"Click to Analyze Resume"}</motion.button>)}
            ...
            text-lg font-semibold transition duration-300 shadow-md">{loading?"starting...":"Start Interview"}</motion.button>
          </div>
