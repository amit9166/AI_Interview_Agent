...
export async function generateQuestion(req,res){
    try {
        ...
        if(!role || !experience || !mode){
            return res.status(400).json({message:"Role, Experence and Mode are required."})
        }
...
export async function finishInterview(req,res){
    try {
        ...
        interview.questions.forEach((q)=>{
            totalScore=totalScore+q.score||0;
            totalConfidence=totalConfidence+q.confidence ||0;
            totalCommunication=totalCommunication+q.communication ||0;
            totalCorrectness=totalCorrectness+q.correctness ||0;
        })
